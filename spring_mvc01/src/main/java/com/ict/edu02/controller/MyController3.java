...
public class MyController3 {
	...
	// 서비스와 같이 다른 곳에서 만들어진 객체를 가져와서 사용하게 해주는 어노테이션
	// @Inject : java에서 지원
	...
	private MemberService memberService;
	  ...
	  // @PostMapping("/calc2") public ModelAndView calc2Exec(CalcVO cvo) { // 파라미터값을
	  // CalcVO로 설정하면 CalcVO에 파라미터들이 알아서 넣어진다.(변수와 파라미터이름이 같아야 한다.) ModelAndView mv =
	  // new ModelAndView("day02/result");
	  
	  // 파라미터가 저장되었는지 확인 //System.out.println(cvo.getS1());
	  
	  // 일처리는 service 에서 int result = calcService.getCalc(cvo);
	  
	  // mv.addObject("cvo", cvo); 
	  // mv.addObject("result", result); return mv; }
	 
	@PostMapping("/calc2")
	// 파라미터값을 CalcVO로 설정하면 CalcVO에 파라미터들이 알아서 넣어진다.(변수와 파라미터이름이 같아야 한다.)
