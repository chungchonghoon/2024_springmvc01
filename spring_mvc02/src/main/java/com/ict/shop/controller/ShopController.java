...
public class ShopController {
	...
	public ModelAndView getShopAddCart(@ModelAttribute("hshop_idx") String shop_idx, HttpSession session) {
		try {
			...
			if (loginchk.equals("ok")) {
					...
					// 카트 리스트에 사용자의 m_id와 제품번호가 있는지(수량 +1) 없는지(카트에 추가) 판별하자
	...
	public ModelAndView getShopAddOK(ShopVO svo, HttpServletRequest request) {
				try {
						...
						file_l.transferTo(new File(path, svo.getP_image_s()));
